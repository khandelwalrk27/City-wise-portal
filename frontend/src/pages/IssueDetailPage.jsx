...
import { MapPin, Building2, Shield, Calendar, AlertTriangle, Upload, CheckCircle2, Camera } from 'lucide-react';
...
export default function IssueDetailPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-bold text-slate-400">#ISSUE-{issue.id}</span>
          ...
          {isAuthorityOfficer && (
            <button
              ...
              className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-md transition"
            >
        ...
        <div className="flex flex-wrap items-center gap-6 text-xs text-slate-400 border-t border-slate-800/80 pt-3">
          <div className="flex items-center gap-1.5">
            <Building2 className="w-4 h-4 text-amber-400" />
            Ward: <strong className="text-slate-200">{issue.ward_name || 'Jaipur Ward'}</strong>
          </div>
          <div className="flex items-center gap-1.5">
            <Shield className="w-4 h-4 text-purple-400" />
            Department: <strong className="text-slate-200">{issue.authority_name || 'Municipal Department'}</strong>
          </div>
          <div className="flex items-center gap-1.5">
            <Calendar className="w-4 h-4 text-indigo-400" />
            Reported Date: <strong className="text-slate-200">{new Date(issue.created_at).toLocaleDateString()}</strong>
          </div>
        </div>
      </div>
      ...
      {duplicateGroup && (
        <div className="p-4 bg-amber-950/30 border border-amber-500/30 rounded-xl flex items-center gap-3 text-amber-300 text-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-3">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Complaint Details</h3>
            <p className="text-xs text-slate-200 leading-relaxed whitespace-pre-line">{issue.description}</p>
          </div>
          ...
          {/* Timeline History */}
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Status History & Audit Trail</h3>
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
            ...
            <p className="text-xs text-slate-200 font-semibold">{issue.address || 'Jaipur'}</p>
          ...
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-2 text-xs">
            <h3 className="font-bold text-white mb-2">Responsible Authority Office</h3>
            <div className="text-slate-300">Department: {issue.authority_department}</div>
            <div className="text-slate-300">Email: {issue.authority_email}</div>
            <div className="text-slate-300">Phone: {issue.authority_phone}</div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAuthorityStatusUpdate} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Officer Remarks / Work Summary</label>
                <textarea
                  rows={3}
                  required
                  value={remarks}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Capture Live Resolution Proof</label>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-800">
                ...
                <button
                  type="submit"
                  disabled={updating}
                  className="px-5 py-2 bg-purple-600 text-white text-xs font-bold rounded-lg hover:bg-purple-500 disabled:opacity-50"
                >
